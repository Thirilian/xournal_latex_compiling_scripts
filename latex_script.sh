...
if grep -q '%%%' "$TEX_FILE"; then #If %%% was written to go to error-display mode
    konsole --noclose -e bash -c "echo 'pute' && less -R '$TEMP_VALID_TEX'"
    ...
    pdflatex -interaction=nonstopmode "$TEX_FILE"
        konsole --noclose -e bash -c "echo 'ducon' && less -R '$TEX_FILE'"
